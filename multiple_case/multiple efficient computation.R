...
simudata = mclapply(1:3,data_gene,rhoxu1 = 0,rhoxu2 = 0,rhoyu1 = 0,rhoyu2 = 0,m = 20,mc.cores=1)
  ...
  suffi_method = function(beta1_mle,beta2_mle,data){
    ...
    for (cl in 1:m){
      ...
      for (sub in 1:n[cl]){
        ...
        nr1 = nrow(Vij)
        ccc = lapply(split(Vij,rep(1:ceiling(nr1/10000), each=10000, length.out=nr1)),matrix, ncol=n[cl])
        ########## calculate numerator
        num_chuncks = function(dataid){
          sum_num = rep(-99,dim(ccc[[dataid]])[1])
          sum_astar = rep (-99,n[cl]-1)
          for (k in 1:dim(ccc[[dataid]])[1]){
            astar_j = ccc[[dataid]][k,-sub]
        ...
        tic()
        ddd = mclapply(1:length(ccc),num_chuncks,mc.cores = 1)
        toc()
        Isub = rep(-99,3)
        ...
        num = exp(num_sub)*sum(unlist(ddd))
