...
data_gene = function(dataid,rhoxu1 = 0,rhoxu2 = 0,rhoyu1 = 0,rhoyu2 = 0,m = 100){
  n = as.matrix(sample(3:20,m,replace=T,prob=rep(1/18,18)))
  ...
  pstrue[,1] = apply(data[,4:7],1,ps0) # prob assign to group0
  pstrue[,2] = apply(data[,4:7],1,ps1) # prob assign to group1
  pstrue[,3] = 1-pstrue[,1]- pstrue[,2] # prob assign to group2
...
for (cl in 1:m){
  ...
  denom_func=function(k){
    ...
    Ib_func=function(q){
      ...
      sum_abar = Iabar%*%c(data_den[q,]%*%beta1_mle,data_den[q,]%*%beta2_mle,0)
      return(sum_abar)
    }
  ...
  for (sub in 1:n[cl]){
    ...
    for (k in 1:dim(Vij)[1]){
      ...
      sum_astar= Iasta%*%c(data_den[q,]%*%beta1_mle,data_den[q,]%*%beta2_mle,0)
      return(sum_astar)
    }
    ...
    # num_sub = Isub%*%d_need
    num_sub = Isub%*%c(data_c[sub,1:3]%*%beta1_mle,data_c[sub,1:3]%*%beta2_mle,0)
...
ps_suffi = prop_ps[-1,]
toc()
data_obs$ps_suffi_est = ps_suffi
data_obs$ps_true = ps_true
y0_suffi = sum(data_obs[data_obs$A==0,]$y_obs/data_obs[data_obs$A==0,]$ps_suffi_est)/((sum(1/data_obs[data_obs$A==0,]$ps_suffi_est)))
y1_suffi = sum(data_obs[data_obs$A==1,]$y_obs/data_obs[data_obs$A==1,]$ps_suffi_est)/((sum(1/data_obs[data_obs$A==1,]$ps_suffi_est)))
y2_suffi = sum(data_obs[data_obs$A==2,]$y_obs/data_obs[data_obs$A==2,]$ps_suffi_est)/((sum(1/data_obs[data_obs$A==2,]$ps_suffi_est)))
cbind(y1_suffi-y0_suffi,y2_suffi-y0_suffi,y2_suffi-y1_suffi)
########### formula based version 2 ##############
...
ps_suffi3 = unlist(lapply(c(1:m), clus_func,data_obs))
################## approximate resample based ##############
prop_ps = 0
for (cl in 1:m){
  data_c = as.matrix(data_obs[data_obs$clu==cl,c(3:5,6)]) ## x1,x2,A
  ########### calculate denominator
  data_den = data_c[,1:3]
  trt_clu = data[data$clu==cl,]$A # trt assignments within cluster
  Vbar = t(replicate(100,sample(trt_clu,n[cl],FALSE)))
  denom_func=function(k){
    abar = Vbar[k,]
    Ib_func=function(q){
      Iabar = matrix(-99,nrow=1,ncol=3)
