tic()
############# Run based on 1 vs 02 ##################
data_new1=data
data_new1$A_relabel1=ifelse(data_new1$A==1,1,0) ## group 1=1,others(0&2)=0
suffi_new = suffi[,1] ## number of group 1 within each cluster
...
############## rerun it again only based on 0&2 groups ############
data_bc = data[data$A==0|data$A==2,]
data_bc$A_relabel2=ifelse(data_bc$A==0,1,0) ## group 0=1,group2=0
suffi_new2 = rep(-99,m) ## number of group 0 within each cluster
for (i in 1:m){
...
index_bc = which(data_new1$A!=1) ## index of units in group 0 and 2
ps_suffi3=ps_suffi_A
ps_suffi3[index_bc] = ps_suffi_A[index_bc]*ps_suffi_BC ## update pscore for group1 and 2
toc()
plot(ps_suffi,ps_suffi2)
plot(ps_suffi,ps_suffi3)
